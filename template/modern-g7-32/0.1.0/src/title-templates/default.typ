...
#import "../utils.typ": sign-field,sign-field-nodata, fetch-field
...
#let template(
    ...
    per-line(
        ...
        (value: subject, when-present: subject),
        (
            value: [(#stage.type)],
    ...
    if manager.name != none {
        sign-field-nodata(manager.at("name"), [Руководитель НИР,\ #manager.at("position")])
    }

    if performer != none {
        sign-field-nodata(performer.at("name", default: none), [Исполнитель НИР,\ #performer.at("position", default: none)], part: performer.at("part", default: none))
    }
